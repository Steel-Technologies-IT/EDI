...
async function insert846Detail(pool, index, InterchangeControl, ProductItem, HeaderNameAddress, InventoryHandoffHeader, flag, totalPieces = null, totalWeight = null, MillHeat) 
{
 try {
  ...
  let gaugIN = ["ED", "E8", "EM", "E7", "IN"].includes(ProductItem.prd_x12gaugeum) ? ProductItem.prd_gaugesize : ProductItem.prd_x12gaugeum === 'EM' ? (ProductItem.prd_gaugesize / 25.4) : null;
  gaugIN = await limitDecimals(gaugIN, 4);
  let widthIN = ["IN", "MM", "MB", "M2", "MZ", "MY"].includes(ProductItem.prd_x12widthum) ? ProductItem.prd_width : ProductItem.prd_x12widthum === 'MM' ? (ProductItem.prd_width / 25.4) : null;
