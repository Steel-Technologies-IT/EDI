...
async function transformI863(pool, key) {
  ...
  resetAddRowTracker();
    ...
    const result3 = await pool.query('SELECT * FROM "863_SNF_Measure" WHERE msr_key = $1', [key]);
    let SNF_Measurements = result3.rows;

    const result4 = await pool.query('SELECT * FROM "863_SNF_Names" WHERE name_key = $1', [key]);
...
try {
    ...
    await insert863InvexInbound(pool, newHeader, newDetails, newMeasurements, newNames);
}
