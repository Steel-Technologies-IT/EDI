...
async function insertInterchangeControlFromJson(pool, data, flow, filePath) {
    try {
        ...
        const icValues = [
            flow, // ictl_type
            null, // ictl_key
            ic.CompanyID, // ictl_companyid
            ic.SenderInterchangeIDQualifier, // ictl_senderinterchangeidqualifier
            ic.SenderInterchangeID, // ictl_senderinterchangeid
            ic.EDIXControlNumber, // ictl_edixcontrolnumber
            ic.ReceiverInterchangeIDQualifier, // ictl_receiverinterchangeidqualifier
            ic.ReceiverInterchangeID, // ictl_receiverinterchangeid
            ic.CreatedDateTime, // ictl_createddatetime
            ic.AlternateInterchangeNumber, // ictl_alternateinterchangenumber
            ic.Status, // ictl_status
            // ic.SenderBranchInterchangeIDQualifier, // NOT IN TABLE
            // ic.SenderBranchInterchangeID, // NOT IN TABLE
            // ic.INVEXBranchCode, // NOT IN TABLE
            flow // ictl_flow_flag
        ];
        try {
        ...
        if (Array.isArray(ic.TransactionSet)) {
            ...
            for (const sh of allShipmentHeaders) {
                const shValues = [
                    null, // tres_type
                    null, // tres_key
                    sh.TransactionReference, // tres_transactionreference
                    sh.ManifestNumber, // tres_manifestnumber
                    sh.VendorShipmentReference, // tres_vendorshipmentreference
                    sh.ShippingDateTime, // tres_shippingdatetime
                    sh.TransactionSetPurposeCode, // tres_transactionsetpurposecode
                    // sh.SomeOtherField, // NOT IN TABLE
                    'I' // tres_flow_flag
                ];
                try {
                    ...
                    console.error('Error inserting ShipmentHeaderTestResult:', err);
                }
            }
            ...
            for (const hna of allHeaderNameAddresses) {
                const hnaValues = [
                    null, // hdna_type
                    null, // hdna_key
                    hna.AddressType, // hdna_addresstype
                    hna.IdentificationCodeQualifier, // hdna_identificationcodequalifier
                    hna.IdentificationCode, // hdna_identificationcode
                    hna.NameLine1, // hdna_nameline1
                    hna.NameLine2, // hdna_nameline2
                    hna.AddressLine1, // hdna_addressline1
                    hna.AddressLine2, // hdna_addressline2
                    hna.AddressLine3, // hdna_addressline3
                    hna.City, // hdna_city
                    hna.PostalCode, // hdna_postalcode
                    hna.CountryCode, // hdna_countrycode
                    hna.StateProvinceCode, // hdna_stateprovincecode
                    hna.TelAreaCode, // hdna_telareacode
                    hna.TelNumber, // hdna_telnumber
                    hna.TelExtension, // hdna_telextension
                    hna.FaxAreaCode, // hdna_faxareacode
                    hna.FaxNumber, // hdna_faxnumber
                    hna.FaxExtension, // hdna_faxextension
                    // hna.SomeOtherField, // NOT IN TABLE
                    'I' // hdna_flow_flag
                ];
                try {
                    ...
                    console.error('Error inserting HeaderNameAddress:', err);
                }
            }
            ...
            for (const sitr of allShipmentItems) {
                const sitrValues = [
                    null, // sitr_type
                    null, // sitr_key
                    sitr.ReferenceLineNumber, // sitr_referencelinenumber
                    null, // sitr_invexordernumber
                    null, // sitr_externalordernumber
                    null, // sitr_externalorderitem
                    sitr.ExternalRelease, // sitr_externalorderrelease
                    null, // sitr_externalorderdate
                    null, // sitr_externalcontactnumber
                    sitr.EndUserPO, // sitr_enduserpo
                    // sitr.SomeOtherField, // NOT IN TABLE
                    'I' // sitr_flow_flag
                ];
                try {
                    ...
                    console.error('Error inserting ShipmentItemTestResult:', err);
                }
            }

            // Updated extraction logic for new JSON structure
            // TransactionSet > ShipmentHeaderTestResult > ShipmentItemTestResult > ProductItem
            ...
            // Move all insert operations to the bottom after extraction
            // TransactionSet Insert
            for (const ts of ic.TransactionSet) {
                const tsValues = [
                    null, // txs_type
                    null, // txs_key
                    ts.TransactionSetControlNumber, // txs_transactionsetcontrolnumber
                    ts.EDIStandardsOrganizationTransactionSet, // txs_edistandardsorganizationtransactionset
                    ts.EDIStandardsOrganization, // txs_edistandardsorganization
                    // ts.SomeOtherField, // NOT IN TABLE
                    null, // txs_status
                    'I' // txs_flow_flag
                ];
                try {
                    ...
                    console.error('Error inserting TransactionSet:', err);
                }
            }

            // ShipmentHeaderTestResult Insert
            for (const sh of allShipmentHeaders) {
                const shValues = [
                    null, // tres_type
                    null, // tres_key
            ...
            for (const pi of allProductItems) {
                ...
                try {
                    ...
                    console.error('Error inserting ProductItem:', err);
                }
            }
            ...
            for (const chem of allChemistry) {
                ...
                try {
                    ...
                    console.error('Error inserting Chemistry:', err);
                }
            }

            // PhysicalTests Insert
            for (const pt of allPhysicalTests) {
                ...
                try {
                    ...
                    console.error('Error inserting PhysicalTests:', err);
                }
            }
        }
    } catch (err) {
        console.error('Error in insertInterchangeControlFromJson:', err);
    }
}
