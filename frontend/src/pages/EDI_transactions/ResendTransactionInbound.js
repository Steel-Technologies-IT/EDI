...
const TableView = () => {
    ...
    const handleResend = useCallback(async (record) => {
        try {
            ...
            const resp = await fetch(`${process.env.REACT_APP_HOST}/EDI_Tables/ResendTransaction`, {
                method: 'POST',
   ...
   const fetchTableData = useCallback(async (tableName) => {
        ...
        try {
            ...
            const columnsResponse = await fetch(`${process.env.REACT_APP_HOST}/EDI_Tables/Tables/${encodeURIComponent(tableName)}/ColumnsInfo`);
            ...
            if ((recordsData.records?.length ?? 0) > 0 && /_SNF_Header$/i.test(tableName)) {
                ...
                if (hasDetail) {
                    ...
                    const detailResp = await fetch(`${process.env.REACT_APP_HOST}/EDI_Tables/Tables/${encodeURIComponent(detailTable)}/Records?limit=all`);
    ...
    useEffect(() => {
        const run = async () => {
            try {
                ...
                const url = `${process.env.REACT_APP_HOST}/EDI_Tables/Tables/${encodeURIComponent(detailTable)}/Records?limit=all&searchColumn=${encodeURIComponent('dtl_mcoil')}&searchTerm=${encodeURIComponent(coilSearch.trim())}`;
    ...
    useEffect(() => {
        const run = async () => {
            try {
                ...
                const url = `${process.env.REACT_APP_HOST}/EDI_Tables/Tables/${encodeURIComponent(detailTable)}/Records?limit=all&searchColumn=${encodeURIComponent('dtl_heat')}&searchTerm=${encodeURIComponent(heatSearch.trim())}`;
                const resp = await fetch(url);
                const data = await resp.json();
