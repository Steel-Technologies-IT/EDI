...
function InboundRulesTable({
    setColumnFilters,
    columnFilters,
    displayedRules = [], // Default empty array
    showFilters,
    clearAllFilters,
    handleInsert,
    handleEdit,
    ...
    const totalPages = Math.ceil(displayedRules.length / rowsPerPage);
    const paginatedRules = displayedRules.slice(
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                    {(!paginatedRules || paginatedRules.length === 0) ? (
                        <tr><td colSpan={8} style={{ textAlign: 'center', padding: 16 }}>No rules set, would you like to <span style={{ cursor: 'pointer', color: '#1976d2' }} onClick={handleInsert}>add one</span>?</td></tr>
